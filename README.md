...
- **数据驱动作物清单**:`config/simudelight/crops.json`；
- **支持的种植形态**(crops.json 的 `kind` 字段):
  - `full` 满铺 CropBlock(卷心菜/洋葱/番茄/芦笋/辣椒/花生/红薯/蔬菜乐事全家/乡村乐事/芋泥乐事/菠萝/黄瓜茄子等)；
  - `paddy` 水田水稻(稻种在作物格下一层;低洼水田把农田盒放进坑里即可;成熟收上层稻穗)；
  - `tall` 旱地两段(玉米)；
  - `watertop` 水面作物(蔓越莓)；
  - `soil` 专用介质 CropBlock(洞穴胡萝卜,需沃土)；
  - `colony` 蘑菇菌落(棕/红蘑菇，需沃土):农夫铺沃土种菌落,成熟采 3 个蘑菇留株续长；
